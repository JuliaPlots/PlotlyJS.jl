...
function plot{T<:Number,T2<:Number}(x::AbstractArray{T}, y::AbstractArray{T2},
                                    l::Layout=Layout(); kind="scatter", kwargs...)
    plot(GenericTrace(x, y; kind=kind, kwargs...), l)
end
...
function plot(f::Function, x0::Number, x1::Number, l::Layout=Layout();
    ...
    x = linspace(x0, x1, 50)
    y = [f(_) for _ in x]
    plot(GenericTrace(x, y; kwargs...), l)
end
...
```
"""
function plot(args::Tuple{AbstractVector,AbstractVector,Vararg{Pair}}...; kwargs...)
    traces = [GenericTrace(a[1], a[2]; a[3:end]...) for a in args]
    plot(traces, Layout(;kwargs...))
end
